...
const sendRequest = async (req, res) => {
  ...
  try {
    ...
    const serviceProvider = await ServiceProvider.findOne({ phoneNumber: serviceProviderPhone });

    if (!user || !serviceProvider) {
      return res.status(404).json({ message: 'User or Service Provider not found' });
    }
...
const registerUser = async (req, res) => {
  const { username, password, phoneNumber } = req.body; // Add phoneNumber
  try {
    const user = new User({ username, password, phoneNumber }); // Include phoneNumber
    await user.save();
...
const registerServiceProvider = async (req, res) => {
  ...
  try {
    const serviceProvider = new ServiceProvider({ username, password, serviceType, phoneNumber }); // Include phoneNumber
    await serviceProvider.save();
    res.status(201).send({ message: 'Service Provider registered successfully' });
  } catch (error) {
    res.status(400).send({ error: 'Registration failed', details: error.message });
  }
};
...
const loginServiceProvider = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: serviceProvider._id }, 'secretkey', { expiresIn: '1h' });
...
module.exports = { registerUser, loginUser, registerServiceProvider, loginServiceProvider,
  sendRequest,
  acceptRequest,
  rejectRequest, };